function [ret] = GauStu_Granger_regress(X,nlags,STATFLAG)
nobs = size(X,2);
nvar = size(X,1);
...
for ii=1:nvar
    ...
    [beta(:,ii),err] = GauStu_Estimate(regressors, xdep);   
...
for ii=1:nvar
    ...
    for jj=1:length(caus_inx)
        ...
        [beta_r,err] = GauStu_Estimate(regressors, xdep);   
        temp_r = xdep-regressors*beta_r;
        RSS0(ii,caus_inx(jj)) = err;
        S(ii,caus_inx(jj)) = err;
